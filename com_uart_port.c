...
int main (int argc, char *argv[])
{
	...
	for(int i=0;i<10;i++)
	{
		...
		fd = open("/tmp/p_fifo",O_RDONLY);

		int n = read(fd,buf,128);
		printf("n is %d and buf is %s\n",n,buf);
		close(fd);

		fd = open("/tmp/p_fifo",O_WRONLY);
		char msg[]="hi hello world";
		int m = write(fd,msg,strlen(msg));
